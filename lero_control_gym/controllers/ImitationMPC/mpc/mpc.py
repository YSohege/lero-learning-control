    ...
    def forward(self, x_init, cost, dx):
        ...
        if self.detach_unconverged:
            if max(best['full_du_norm']) > self.eps:
                ...
                Ix = Variable(
                    I.unsqueeze(0).unsqueeze(2).expand_as(x)).type_as(
                    x.data)
                Iu = Variable(
                    I.unsqueeze(0).unsqueeze(2).expand_as(u)).type_as(
                    u.data)
...
""")
            ...
            for t in range(self.T):
                ...
                grad = torch.autograd.grad(
                    cost.sum(), tau_t, create_graph=True, retain_graph=True)[0]
                hessian = list()
    ...
    def linearize_dynamics(self, x, u, dynamics, diff):
        ...
        if self.grad_method == GradMethods.ANALYTIC:
            ...
            # we can efficiently compute grad_input for every time step at
            # once.
                    ...
                    if self.grad_method in [GradMethods.AUTO_DIFF,
                        ...
                        if self.grad_method == GradMethods.ANALYTIC_CHECK:
                            ...
                            if torch.max(
                                torch.abs(
                                    Rt -
                                    Rt_autograd)).data[0] > eps or torch.max(
                                torch.abs(
                                    St -
