...
def LQRStep(n_state,
    ...
    def lqr_backward(ctx, C, c, F, f):
        ...
        for t in range(T - 1, -1, -1):
            ...
            if u_lower is None:
                if n_ctrl == 1 and u_zero_I is None:
                    ...
                    if u_zero_I is None:
                        ...
                        Qt_ux_ = Qt_ux.clone()
                        Qt_ux_[
                            I.unsqueeze(2).repeat(
                                1, 1, Qt_ux.size(2)).bool()] = 0.
                        ...
                        if n_ctrl == 1:
                            ...
                            kt = - \
                                qt_u_.unsqueeze(2).lu_solve(*Qt_uu_LU_).squeeze(2)
            else:
                assert delta_space
                ...
                prev_kt = kt
                Qt_ux_ = Qt_ux.clone()
                Qt_ux_[(1 - If).unsqueeze(2).repeat(1,
                                                    1, Qt_ux.size(2)).bool()] = 0
                if n_ctrl == 1:
            ...
            Vtp1 = Qt_xx + Qt_xu.bmm(Kt) + Kt_T.bmm(Qt_ux) + \
            ...
            for t in range(T):
                ...
                if u_lower is not None:
                    ...
                    if delta_u is not None:
                        ...
                        ub = u[t] + delta_u
                        I = lb < lb_limit
                        lb[I] = lb_limit if isinstance(
                            lb_limit, float) else lb_limit[I]
                        I = ub > ub_limit
                        ub[I] = ub_limit if isinstance(
                            lb_limit, float) else ub_limit[I]
                ...
                if isinstance(true_cost, mpc.QuadCost):
                    ...
                    obj = 0.5 * \
                        util.bquad(new_xut, C[t]) + util.bdot(new_xut, c[t])
                else:
    ...
    class LQRStepFn(Function):
        ...
        def backward(ctx, dl_dx, dl_du, temp=None, temp2=None):
            ...
            _mpc = mpc.MPC(
                ...
                n_batch=C.size(1),
                delta_u=None,
                # exit_unconverged=True, # It's really bad if this doesn't
                # converge.
                exit_unconverged=False,
                # It's really bad if this doesn't converge.
                eps=back_eps,
            )
