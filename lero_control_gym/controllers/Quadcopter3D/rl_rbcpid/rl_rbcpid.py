...
class RL_RBCPID(BaseController):
    ...
    def __init__(self,
        ...
        self.TILT_LIMITS = [
            (TILT_LIMITS[0] / 180.0) * 3.14,
            (TILT_LIMITS[1] / 180.0) * 3.14]
        ...
        # initilize distribution with uniform density based on attitude
        # controller set size
        if RBC_DISTRIBUTION_PARAMETERS is None:
            self.alpha = [1] * self.numberControllers
        else:

            if not len(ATTITUDE_CONTROLLER_SET) == len(
                    RBC_DISTRIBUTION_PARAMETERS):
                print("Controller set and Distribution not same size")
                exit()

            self.alpha = RBC_DISTRIBUTION_PARAMETERS
        # print(self.alpha)
        self.quantiles = [1 / self.numberControllers] * \
            self.numberControllers  # must sum to 1
        self.AttitudeControllerDistribution = DirichletDistribution(
            self.alpha, seed=self.random_seed)
    ...
    def run(self,
        ...
        for i in range(iterations):
            ...
            throttle, target_euler, pos_e = self._PIDPositionControl(
                state, target)
            ...
            action = self._supervisoryControl(self.obs, rpms)
    ...
    def step(self, action):
        ...
        # supervisoryObs = np.array([pos_e, target_euler[0] - state[6], target_euler[1] - state[7], target_euler[2] - state[8]])
        # print(supervisoryObs)
        self.control_action = self._supervisoryControl(self.obs, rpms)

        self.obs, self.rew, self.done, self.info = self.env.step(
            self.control_action)

        self.updateResultDict(
            self.obs,
            self.rew,
            self.done,
            self.info,
    ...
    def _PIDPositionControl(self,
        ...
        dest_x_dot = self.PositionController[0][0] * (
            x_error) + self.PositionController[2][0] * (-cur_vel[0]) + self.xi_term
        dest_y_dot = self.PositionController[0][1] * (
            y_error) + self.PositionController[2][1] * (-cur_vel[1]) + self.yi_term
        dest_z_dot = self.PositionController[0][2] * (
            z_error) + self.PositionController[2][2] * (-cur_vel[2]) + self.zi_term

        throttle = np.clip(dest_z_dot, self.Z_LIMITS[0], self.Z_LIMITS[1])

        dest_theta = dest_x_dot * \
            math.sin(cur_orientation[2]) - dest_y_dot * math.cos(cur_orientation[2])
        dest_phi = dest_x_dot * \
            math.cos(cur_orientation[2]) + dest_y_dot * math.sin(cur_orientation[2])

        # --------------------
        # get required attitude states
        dest_gamma = self.yaw_target  # fixed at 0
        dest_theta, dest_phi = np.clip(
            dest_theta, self.TILT_LIMITS[0], self.TILT_LIMITS[1]), np.clip(
            dest_phi, self.TILT_LIMITS[0], self.TILT_LIMITS[1])
    ...
    def _PIDAttitudeControl(self,
        ...
        gamma_dot_error = (self.YAW_RATE_SCALER *
                           self._wrap_angle(dest_gamma - gamma)) - gamma_dot
        ...
        for controller in self.AttitudeControllerSets:
            ...
            z_val = np.clip(
                z_val,
                self.YAW_CONTROL_LIMITS[0],
                self.YAW_CONTROL_LIMITS[1])
            ...
            rpm = np.clip([m1, m2, m3, m4], self.MOTOR_LIMITS[0],
                          self.MOTOR_LIMITS[1])
    ...
    def _supervisoryControl(self, obs, rpms):
        # predict the blending distribution parameters using the trained
        # network
    ...
    def printState(self, obs, reward, done, info, action, target_euler):
        ...
        print("Attitude Error [Roll, Pitch, Yaw] =" +
              str(self.attitudeErrors[-1]))
