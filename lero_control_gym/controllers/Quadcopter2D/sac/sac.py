...
from lero_control_gym.utils.logging import ExperimentLogger
from lero_control_gym.utils.utils import get_random_state, set_random_state, is_wrapped
from lero_control_gym.envs.env_wrappers.vectorized_env import make_vec_envs
from lero_control_gym.envs.env_wrappers.vectorized_env.vec_env_utils import _flatten_obs, _unflatten_obs
from lero_control_gym.envs.env_wrappers.record_episode_statistics import RecordEpisodeStatistics, VecRecordEpisodeStatistics
from lero_control_gym.math_and_models.normalization import BaseNormalizer, MeanStdNormalizer, RewardStdNormalizer

from lero_control_gym.controllers.base_controller import BaseController
from lero_control_gym.controllers.Quadcopter2D.sac.sac_utils import SACAgent, SACBuffer
...
class SAC(BaseController):
    ...
    def save(self, path, save_buffer=True):
        ...
        if self.training:
            ...
            # but intermediate checkpoint shoud not, to save storage (buffer is
            # large)
            if save_buffer:
    ...
    def train_step(self, **kwargs):
        ...
        if self.total_steps < self.warm_up_steps:
            ...
            with torch.no_grad():
                act = self.agent.ac.act(
                    torch.FloatTensor(obs).to(
                        self.device), deterministic=False)
        ...
        if len(terminal_obs) > 0:
            terminal_obs = _unflatten_obs(
                self.obs_normalizer(
                    _flatten_obs(terminal_obs)))

        # collect the true next states and masks (accounting for time
        # truncation)
