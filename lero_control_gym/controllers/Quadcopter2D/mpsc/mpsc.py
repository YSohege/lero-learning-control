...
from lero_control_gym.controllers.Quadcopter2D.mpc.mpc_utils import get_cost_weight_matrix, discretize_linear_system
from lero_control_gym.controllers.Quadcopter2D.mpsc.mpsc_utils import compute_RPI_set, pontryagin_difference_AABB, ellipse_bounding_box
from lero_control_gym.envs.constraints import QuadraticContstraint, ConstraintList, ConstrainedVariableType
...
class MPSC(BaseController):
    ...
    def reset_constraints(self,
        ...
        if len(constraints_list.input_state_constraints) > 0:
            raise NotImplementedError(
                'MPSC cannot handle combined state input constraints yet.')
    ...
    def set_linear_dynamics(self):
        ...
        discrete_dfdx, discrete_dfdu = discretize_linear_system(
            dfdx, dfdu, self.dt)
    ...
    def compute_lqr_gain(self):
        ...
        self.lqr_gain = -np.dot(np.linalg.inv(self.R + np.dot(btp,
                                self.discrete_dfdu)), np.dot(btp, self.discrete_dfdx))
    ...
    def learn(self,
        ...
        for i in range(self.n_samples):
            ...
            x_next_linear = self.linear_dynamics_func(
                x0=init_state - self.X_LIN, p=u - self.U_LIN)['xf'].toarray()
        ...
        self.omega_constraint = QuadraticContstraint(
            self.env, P, 1.0, constrained_variable=ConstrainedVariableType.STATE)
    ...
    def tighten_state_and_input_constraints(self):
        ...
        if self.omega_AABB_verts is not None:
            K_omega_AABB_verts_raw = (
                self.lqr_gain @ self.omega_AABB_verts.T).T
            # Take the outermost values.
            self.K_omega_AABB_verts = np.vstack(
                (np.max(K_omega_AABB_verts_raw), np.min(K_omega_AABB_verts_raw)))
            ...
            self.U_vertices = np.array([np.atleast_1d(
                input_constraint.upper_bounds), np.atleast_1d(input_constraint.lower_bounds)])
            ...
            X_vertices_raw = [
                (state_constraints.upper_bounds[i],
                 state_constraints.lower_bounds[i]) for i in range(
                    self.model.nx)]
            self.X_vertices = np.clip(
                np.vstack(
                    list(
                        product(
                            *X_vertices_raw))), -100, 100)
            self.tightened_state_constraint_verts, tightened_state_constraint_func = pontryagin_difference_AABB(
                self.X_vertices, self.omega_AABB_verts)
    ...
    def setup_optimizer(self):
        ...
        # Constraints (currently only handles a single constraint for state and
        # input).
        ...
        # Real input (5.f).
        opti.subject_to(u_tilde == v_var[:, 0] +
                        self.lqr_gain @ (x - z_var[:, 0]))
    ...
    def solve_optimization(self,
        ...
        try:
            sol = opti.solve()
            x_val, u_val, u_tilde_val = sol.value(
                z_var), sol.value(v_var), sol.value(u_tilde)
            self.z_prev = x_val
            self.v_prev = u_val
    ...
    def select_action(self,
        ...
        if self.rl_controller is not None:
            ...
            u_L = 2 * np.sin(0.01 * np.pi * self.time_step) + \
                0.5 * np.sin(0.12 * np.pi * self.time_step)
    ...
    def close_results_dict(self):
        ...
        self.results_dict['uncertified_obs'] = np.vstack(
            self.results_dict['uncertified_obs'])
        self.results_dict['uncertified_actions'] = np.vstack(
            self.results_dict['uncertified_actions'])
